import {
  ...
  InputInterface,
  VaultInterface,
  VaultParamsInterface, BundlerVaultInterface
} from "./faces";
...
export function handle(state: StateInterface, action: ActionInterface): { state: StateInterface } | { result: any } {
  ...
  const bundlerVault: BundlerVaultInterface = state.bundlerVault;
  ...
  if (input.function === "addBundler") {
    const target = isArweaveAddress(caller);
    const { initialTxId } = input;
    const minimum = settings.get("bundlerMinimum");
    const qty = input.qty ?? minimum;
    // Check locked amount is > 25,000
    if (caller in balances) {
      const balance = balances[target];
      if (balance < minimum) {
        throw new ContractError(`Not enough tokens staked. Required: ${minimum} | Actual: ${balance}`);
      }

      // Deduct from balance
      balances[target] -= qty;

      // Add to bundler vault
      if (target in bundlerVault) {
        bundlerVault[target].balance += qty;
      } else {
        bundlerVault[target] = qty;
      }
    } else {
      throw new ContractError("No tokens in wallet");
    }

    // TODO: Is initialTxId seeded?

    // Add tx id
    state.bundlers[target] = initialTxId;
